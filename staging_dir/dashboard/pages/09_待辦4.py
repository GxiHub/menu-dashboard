...
STATUS_LABEL = {"open": "待辦", "doing": "進行中", "blocked": "觀念", "done": "完成"}
...
st.set_page_config(page_title="待辦4｜菜單儀表板", layout="wide")
user = require_page("待辦4")
log_access(user, "待辦4")
apply_base_css()
...
with h1:
    st.markdown(
        f'<div class="page-title">待辦4</div>'
        f'<div class="page-subtitle">{today_str} &nbsp;·&nbsp; {user_short} &nbsp;·&nbsp; 待辦4</div>',
        unsafe_allow_html=True,
    )
with h2:
...
tasks_all = todo_list(
    ...
    include_done=True,
    sort=sort,
    current_user=user, namespace="todo4",
)
...
with tabs[0]:
    render_task_list([t for t in tasks_all if t["status"] == "open"], "t0")
with tabs[1]:
    render_task_list([t for t in tasks_all if t["status"] == "doing"], "t1")
with tabs[2]:
    render_task_list([t for t in tasks_all if t["status"] == "blocked"], "t2")
with tabs[3]:
    render_task_list([t for t in tasks_all if t["status"] == "done"], "t3")
with tabs[4]:
    render_task_list(tasks_all, "t4")
